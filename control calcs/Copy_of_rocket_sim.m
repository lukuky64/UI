...
% ---------------------------
gravity = -9.81;             % m/s²
acceleration = 20;           % Example burnout time in seconds
apogee = 200;               % Example desired apogee in metres
terminal_velocity = -10;     % Example terminal velocity in m/s
resolution = 320 * 8;          % Number of data points
...
% Run the simulation
data = runSimulation(acceleration, apogee, terminal_velocity, gravity, ...
...
function y = compute_t_b(g, a_b, S_a)
    % Translated from C++ logic to MATLAB
    y = sqrt(S_a / ((0.5 * a_b) + ((a_b * a_b)/(2 * abs(g)))));
end
...
function data = runSimulation(acceleration, apogee, terminal_velocity, gravity, resolution, ...
                              temp_lapse_rate, temperature, h_b, static_pressure, molar_mass, gas_constant)
    % Compute initial acceleration
    burnout_time = compute_t_b(gravity, acceleration, apogee)
